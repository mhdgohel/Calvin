...
Paxos::Paxos(const string& zookeeper_config_file, bool reader) {
  ...
  int wait_count = 0;
  while (zoo_state(zh_) != ZOO_CONNECTED_STATE && wait_count < 100) {
    usleep(100000); // 100ms
    wait_count++;
  }
  if (zoo_state(zh_) != ZOO_CONNECTED_STATE) {
    printf("Timed out waiting for Zookeeper connection.\n");
  }

  printf("Zookeeper init successful. Checking root node...\n");
...
void Paxos::SubmitBatch(const string& batch_data) {
  ...
  printf("Submitting batch to Zookeeper...\n");
