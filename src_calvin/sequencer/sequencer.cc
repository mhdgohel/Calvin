...
Sequencer::Sequencer(Configuration* conf, Connection* connection, Connection* sync_connection, Client* client,
                     Storage* storage)
    : epoch_duration_(0.01), configuration_(conf), connection_(connection), sync_connection_(sync_connection),
      client_(client), storage_(storage), deconstructor_invoked_(false) {
  pthread_mutex_init(&mutex_, NULL);
  // Start Sequencer main loops running in background thread.

// CPU affinity removed
...
void Sequencer::RunWriter() {
  ...
  fprintf(stderr, "RunWriter started.\n");
  Spin(1);
  ...
  synchronization_message.set_destination_channel("sequencer_sync");
  ...
  fprintf(stderr, "Node %d: Waiting for synchronization... (counter=%d, total=%lu)\n", configuration_->this_node_id, synchronization_counter, configuration_->all_nodes.size());
  while (synchronization_counter < configuration_->all_nodes.size()) {
    ...
    if (sync_connection_->GetMessage(&synchronization_message)) {
      ...
      fprintf(stderr, "Node %d: Received sync message. Counter now %d\n", configuration_->this_node_id, synchronization_counter);
    } else {
      Spin(0.001);
    }
  }
  ...
  for (int batch_number = configuration_->this_node_id;
    ...
    // Send this epoch's requests to ALL nodes (including self).
#ifdef PAXOS
    string batch_string;
    batch.set_destination_node(-1);
    batch.SerializeToString(&batch_string);
    fprintf(stderr, "Node 0: Submitting batch %d to Paxos\n", batch_number);
    ...
    for (map<int, Node*>::iterator it = configuration_->all_nodes.begin();
      ...
      batch.set_destination_node(it->first);
      connection_->Send(batch);
    }
#endif
  }
...
void Sequencer::RunReader() {
  ...
  while (!deconstructor_invoked_) {
    // Get batch from network (leader).
    string batch_string;
    MessageProto batch_message;
#ifdef PAXOS
    paxos.GetNextBatchBlocking(&batch_string);
#else
    if (connection_->GetMessageBlocking(&batch_message, 1.0)) {
      batch_message.SerializeToString(&batch_string);
    } else {
      continue;
    }
#endif
